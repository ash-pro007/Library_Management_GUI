...
class AppTk:
    ...
    def issue_book(self):
        ...
        def add_issue_record():
            book_isbn = ent_book_isbn.get().strip()
            student_id = ent_student_id.get().strip()
            ...
            cur0 = db.cursor()
            cur0.execute(f'SELECT * FROM book_record where isbn="{book_isbn}" and id={student_id}')

            can_book_be_issued = True

            for i in cur0:
                print('inside for i[5]= ', i[5])
                if i[5] is None or i[5] == '':
                    can_book_be_issued = False

            cur0.execute(f'SELECT name FROM students WHERE id="{student_id}"')
            student_name = ''
            for i in cur0:
                student_name = i[0]

            cur0.execute(f'SELECT name FROM books WHERE isbn="{book_isbn}"')
            book_name = ''
            for i in cur0:
                book_name = i[0]
            ...
            # Checking weather the book is already issued or not and if it is then is it returned or not.
            elif not can_book_be_issued:
                show_message("This book is already issue to student and hasn't been submitted\n"
                             "This book cannot be issued to student", 'showerror', 'Date Error')
                ...
                if all_ok == 2:
                    ...
                    ans = get_conformation(f'Do you want issue "{book_name}" to "{student_name}"')
                    if ans:
        ...
        btn_go_back = ttk.Button(frame_inside, text='Go Back', command=lambda: (self.main_options(),
                                                                                frame_issue_book.destroy()))
        btn_go_back.grid(row=5, column=1, ipadx=self.screen_width // 50, padx=self.screen_width // 50,
                         pady=self.screen_height // 20, sticky=W)
    ...
    def submit_book(self):
        ...
        btn_get_from_date = ttk.Button(frame_return_date, command=lambda: get_date(cal_returned_data), text="SELECT", )
        btn_get_from_date.grid(row=0, column=2, padx=10)

        def add_submit_record():
            student_id = ent_student_id.get().strip()
            isbn = ent_book_isbn.get().strip()
            ...
            if is_record_in_db:
                if get_conformation('Do you want to submit the book'):

                    print('Near query')
                    cur.execute(
                        f'UPDATE book_record SET returned_at="{return_date}" where id={student_id} and isbn="{isbn}"')
                    db.commit()
                    self.submit_book()

        btn_return_book = ttk.Button(frame_submit_book, text='Return Book', command=lambda: (add_submit_record(),
                                                                                             get_date(
                                                                                                 cal_returned_data)))

        btn_go_back = ttk.Button(frame_submit_book, text='Go Back Book', command=lambda: (self.main_options()))
    ...
    def manage_books(self):
        ...
        Button(frame_manage_books, text='Go Back', relief=FLAT, background=self.settings['btn_primary_bg_color'],
    ...
    def change_details(self):
        ...
        Label(frame_change_details_option, text='Select an option to change details',
        ...
        def change_by_list():
            ...
            def change_details():
                try:
                    ...
                    btn_add_img = Button(frame_changing_fields, text='Add Image', font=self.settings['primary_font'],
                                         command=get_image)
                    ...
                    btn_add = ttk.Button(frame_changing_fields, text='Change',
                                         command=lambda: (get_date(cal_adding_data),
                                                          verify_book_details(), change_by_list(),
                                                          show_message(
                    ...
                    btn_go_back = ttk.Button(frame_changing_fields, text='Go back',
                                             command=change_by_list)
                    ...
                    btn_add_img.grid(row=7, column=2, padx=self.screen_width // 50, pady=self.screen_height // 100,
                                     sticky=EW)
        ...
        btn_change_by_isbn = Button(frame_change_details_option, text='Change details by INBN', relief=FLAT,
                                    background='White', font=self.settings['primary_font'], activebackground='#66b3ff',
                                    activeforeground='White', command=change_by_isbn)
        btn_change_by_isbn.grid(column=0, row=1, padx=self.screen_width // 10,
                                pady=self.screen_height // 30, )

        btn_change_by_list = Button(frame_change_details_option, text='Change details by List', relief=FLAT,
                                    background='White', font=self.settings['primary_font'], activebackground='#66b3ff',
                                    activeforeground='White', command=change_by_list)
        btn_change_by_list.grid(column=1, row=1, padx=self.screen_width // 10,
    ...
    def track_student(self):
        ...
        def track(coming_from):
            ...
            if all_ok:
                ...
                treeview_history.tag_configure('green', background='#54e367')
                treeview_history.tag_configure('yellow', background='#e3e154')
                treeview_history.tag_configure('red', background='Pink')
                ...
                for tpl2 in list_record:
                    ...
                    # date comparision to show records in different color

                    # Green -> Book successfully submitted
                    # Yellow -> Not submitted but still in notice period
                    # Red -> Submittion date has been reached

                    issued_to_date_sum = 0
                    returned_date_sum = 0
                    today_date_sum = 0

                    today_date = str(datetime.datetime.now())
                    today_date = today_date[2:10]
                    ...
                    for n in tpl2[2].split('/'):
                        try:
                            issued_to_date_sum += int(n)
                        except:
                            pass

                    if tpl2[3] is not None and tpl2[3] != '':
                        for n in tpl2[3].split('/'):
                            try:
                                returned_date_sum += int(n)
                            except:
                                continue

                    for n in today_date.split('/'):
                        try:
                            today_date_sum += int(n)
                        except:
                            pass

                    if today_date_sum >= issued_to_date_sum:
                    ...
                    elif issued_to_date_sum >= returned_date_sum:
                        treeview_history.insert(parent='', index='end',
                                                image=self.lst_all_book_images[count2 - 1],
                                                values=(
                                                    tpl2[0], tpl2[1], tpl2[2], tpl2[3]
        ...
        ttk.Button(frame_track_by_id, text='Track',
                   command=lambda: (track('by_id'))).grid(row=0, column=2, padx=self.screen_width // 50, )
    ...
    def view_books(self, coming_from='None'):
        ...
        for tpl in lst_book_main:
            ...
            cur2 = db.cursor()
            cur2.execute(f'SELECT COUNT(*) FROM book_record WHERE isbn = "{tpl[2]}"')

            available_book = tpl[3]
            for i in cur2:
                available_book = i[0]
        ...
        if coming_from == 'issue_book':
            Button(frame_view_books, text='Go Back', command=lambda: (frame_view_books.destroy(),
